...
The `kotlin-patterns` Helm chart can be used to deploy the `kotlin-patterns` application. The chart will deploy the following resources:
...
The following table lists the configuration parameters of the kotlin-patterns chart, and their default values.
...
| `ingress.rewritePath` | `bool` | `true` | Indicates whether pathPrefix is rewritten by the ingress. <br/> If this is set to `true` then the kotlin-patterns dynamic content and static assets will be served from `/`, otherwise, they will be served from `/$pathPrefix`. |
| `ingress.pathPrefix` | `string` | `""` | The path prefix configured in the ingress for the kotlin-patterns service.<br/> Must be provided when ingress is used. |
| `service.type` | `string` | `"LoadBalancer"` | The service type. |
| `service.port` | `int` | `80` | The port exposed by the service. |
| `deployment.replicaCount` | `int` | `2` | The number of replicas for the kotlin-patterns deployment. |
| `deployment.container.image.repository` | `string` | `kotlin-patterns` | The container image to run in the kotlin-patterns pods. |
...
| `deployment.container.port` | `int` | `"8080"` | The port that kotlin-patterns app listens on. |
...
You can modify the `kotlin-patterns` app by providing new values for the configuration parameter defined in the table above, either directly in the `values.yaml`
file, or via the `--set` switches.

```bash
$ helm upgrade --namespace webapp custom-kotlin-patterns . --set message="This is a different message"
```

### Uninstalling the chart

You can uninstall the `kotlin-patterns` app as follows:

```bash
$ helm uninstall --namespace webapp custom-kotlin-patterns
```
